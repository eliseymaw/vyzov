...
function formatGeo(scope: string, districts: string[], metros: string[]) {
  ...
  if (scope === "district") return districts.length > 0 ? `Районы: ${districts.join(", ")}` : "Районы не указаны"
  if (scope === "metro") return metros.length > 0 ? `Метро: ${metros.join(", ")}` : "Метро не указано"
...
function formatReceiveScope(user: User) {
  ...
  if (user.receive_scope === "district") return `Получаете рассылки по районам: ${(user.districts ?? []).join(", ")}`
  if (user.receive_scope === "metro") return `Получаете рассылки по метро: ${(user.metros ?? []).join(", ")}`
...
export default function InboxPage() {
  ...
  async function loadInbox() {
    ...
    if (userData.inbox_unlocked) {
      const inboxResponse = await authFetch("http://localhost:8000/users/me/inbox")
  ...
  if (loading) {
    return <main className="min-h-screen bg-black p-6 text-white">Загрузка входящих...</main>
  }
    ...
    return (
      <main className="min-h-screen bg-black p-6 text-white">
        <div className="mx-auto mt-20 max-w-2xl rounded-2xl border border-zinc-800 bg-zinc-950 p-8 text-center">
          ...
          <button
            onClick={async () => {
              await authFetch("http://localhost:8000/users/me/top-up", {
                method: "POST",
      ...
      <div className="max-w-3xl">
        ...
        <div className="mt-8 space-y-4">
          {ads.length === 0 ? (
            ...
            ads.map((ad) => (
              <div key={ad.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-5">
                ...
                <div className="mt-4 space-y-2 text-sm text-zinc-400">
                  ...
                  <p>
                    Кому: {formatGender(ad.target_gender)} • Возраст: {ad.target_age_from}–{ad.target_age_to}
                  </p>
                </div>
